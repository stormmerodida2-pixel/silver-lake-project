...
Runs at `http://localhost:5173`. Configure `VITE_API_BASE_URL` (the backend API root) in
`frontend/.env`.
...
5. Set two GitHub repo **variables** (Settings → Secrets and variables → Actions → Variables):
   ...
   frontend with the right API base URL). Optionally also add `GA_MEASUREMENT_ID` (from
   analytics.google.com - Admin → Data Streams → your web stream, format `G-XXXXXXXXXX`) to
   enable Google Analytics; leave it unset to ship with analytics disabled entirely.
