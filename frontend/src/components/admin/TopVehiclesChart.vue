...
<template>
  <div class="relative">
    <svg :viewBox="`0 0 ${WIDTH} ${height}`" class="w-full">
      <g
        v-for="(row, i) in data" :key="row.id"
        tabindex="0" class="cursor-pointer focus:outline-none"
        @mouseenter="hoverIndex = i" @mouseleave="hoverIndex = null"
        @focus="hoverIndex = i" @blur="hoverIndex = null"
      >
        <text :x="LABEL_WIDTH - 10" :y="rowY(i) + BAR_HEIGHT / 2 + 4" text-anchor="end" font-size="12" fill="#cbd5e1">
          {{ truncate(row.name) }}
        </text>
        <rect
          :x="LABEL_WIDTH" :y="rowY(i)" :width="Math.max(barWidth(row.revenue), 3)" :height="BAR_HEIGHT" rx="4"
          :fill="GOLD" :fill-opacity="hoverIndex === i ? 1 : 0.85"
        />
        <text
          :x="LABEL_WIDTH + barWidth(row.revenue) + 8" :y="rowY(i) + BAR_HEIGHT / 2 + 4"
          font-size="11" fill="#94a3b8" style="font-variant-numeric: tabular-nums"
        >
    ...
    >
      ...
      <p class="text-slate-400">{{ data[hoverIndex].bookings }} booking{{ data[hoverIndex].bookings === 1 ? '' : 's' }}</p>
    </div>
