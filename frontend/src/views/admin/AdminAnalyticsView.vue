...
<template>
  <div>
    ...
    <div v-else class="mt-6 space-y-6">
      <section class="rounded-2xl border border-border-subtle bg-surface p-6">
        <div class="flex items-center justify-between">
          <h2 class="text-sm font-semibold uppercase tracking-wide text-accent">Revenue Collected</h2>
          <button
            class="text-xs font-semibold text-foreground-muted hover:text-accent"
          ...
          <table class="w-full text-left text-sm">
            ...
            <tbody class="divide-y divide-border-subtle">
              <tr v-for="month in data.revenue_trend" :key="month.month">
                <td class="py-1.5 pr-4 text-foreground-secondary">{{ monthLabel(month.month) }}</td>
                <td class="py-1.5 text-foreground" style="font-variant-numeric: tabular-nums">
      ...
      <div class="grid gap-6 lg:grid-cols-2">
        <section class="rounded-2xl border border-border-subtle bg-surface p-6">
          <div class="flex items-center justify-between">
            <h2 class="text-sm font-semibold uppercase tracking-wide text-accent">Top Vehicles by Revenue</h2>
            <button
            ...
            <table class="w-full text-left text-sm">
              ...
              <tbody class="divide-y divide-border-subtle">
                <tr v-for="vehicle in data.top_vehicles" :key="vehicle.id">
                  <td class="py-1.5 pr-4 text-foreground-secondary">{{ vehicle.name }}</td>
                  <td class="py-1.5 pr-4 text-foreground-secondary" style="font-variant-numeric: tabular-nums">
                    {{ vehicle.bookings }}
                  </td>
                  <td class="py-1.5 text-foreground" style="font-variant-numeric: tabular-nums">
        ...
        <section class="rounded-2xl border border-border-subtle bg-surface p-6">
          <h2 class="text-sm font-semibold uppercase tracking-wide text-accent">New vs Repeat Customers</h2>
          <div class="mt-4 grid grid-cols-2 gap-4">
            <div>
              <p class="text-xs text-foreground-subtle">Repeat Rate</p>
              <p class="mt-1 text-3xl font-bold text-foreground">{{ data.customers.repeat_rate }}%</p>
            </div>
            <div>
              <p class="text-xs text-foreground-subtle">Total Customers</p>
              <p class="mt-1 text-3xl font-bold text-foreground">{{ data.customers.new + data.customers.repeat }}</p>
            </div>
          </div>

          <div v-if="data.customers.new + data.customers.repeat > 0" class="mt-6">
            <div class="mb-2 flex items-center gap-4 text-xs">
              <span class="flex items-center gap-1.5 text-foreground-secondary">
            ...
            <div class="flex h-6 w-full gap-0.5 overflow-hidden rounded-full bg-page">
              <div
                class="flex items-center justify-center text-[10px] font-semibold text-on-accent"
              ...
              <div
                class="flex items-center justify-center text-[10px] font-semibold text-foreground"
          ...
          <p v-else class="mt-6 text-sm text-foreground-subtle">No confirmed bookings in the last 12 months yet.</p>
        </section>
      </div>
    </div>
  </div>
